...
function Admin() {
  ...
  const handleGuardar = async () => {
    const res = await fetch(`http://localhost:3001/juegos/${editando.id}`, {
      ...
      body: JSON.stringify({
        ...
        genre: editando.genre,
        descripcion: editando.descripcion,
        requisitos: editando.requisitos
      })
    });
    if (res.ok) {
  ...
  const handleAgregarJuego = async (e) => {
    ...
    formData.append('requisitos', nuevoJuego.requisitos);
    ...
    if (res.ok) {
      setNuevoJuego({ title: '', genre: '', price: '', stock: '', release_date: '', desarrollador_id: '', descripcion: '', requisitos: '' });
      setImagenFile(null);
      setPreview(null);
      cargarDatos();
  ...
  const handleCambiarImagen = async (file, id) => {
    ...
    if (res.ok) { cargarDatos(); mostrarMensaje('✅ Imagen actualizada correctamente'); }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="max-w-6xl mx-auto p-8">
        {mensaje && <div className="bg-green-900 text-green-300 px-4 py-3 rounded-lg mb-6 text-sm">{mensaje}</div>}
        ...
        {tab === 'juegos' && (
          <div className="flex flex-col gap-8">
            ...
            <div className="bg-gray-800 rounded-xl p-6">
              <h2 className="text-xl font-bold mb-6 text-yellow-400">➕ Agregar Juego</h2>
              <form onSubmit={handleAgregarJuego} className="flex flex-col gap-4">
                ...
                {/* Fila 2: Precio, Stock, Fecha, Desarrollador */}
                <div className="grid grid-cols-2 gap-4">
                ...
                {/* Fila 3: Descripción y Requisitos */}
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm text-gray-400 mb-1 block">📖 Descripción</label>
                    <textarea value={nuevoJuego.descripcion} onChange={e => setNuevoJuego({...nuevoJuego, descripcion: e.target.value})} placeholder="Descripción del juego..." rows={4} className="w-full bg-gray-700 text-white px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 resize-none" />
                  </div>
                  <div>
                    <label className="text-sm text-gray-400 mb-1 block">💻 Requisitos del sistema</label>
                    <textarea value={nuevoJuego.requisitos} onChange={e => setNuevoJuego({...nuevoJuego, requisitos: e.target.value})} placeholder="SO: Windows 10&#10;RAM: 8GB&#10;GPU: GTX 660..." rows={4} className="w-full bg-gray-700 text-white px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 resize-none" />
                  </div>
                </div>
                {/* Drag & Drop */}
                <div
                  ...
                  className={`border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition-colors ${dragOver ? 'border-yellow-400 bg-yellow-900/20' : 'border-gray-600 hover:border-gray-400'}`}
                >
                  {preview ? (
                  ...
                  <input id="fileInput" type="file" accept="image/*" className="hidden" onChange={e => { const file = e.target.files[0]; if (file) { setImagenFile(file); setPreview(URL.createObjectURL(file)); }}} />
                </div>
                <button type="submit" className="bg-yellow-600 hover:bg-yellow-700 text-white font-semibold py-2 rounded-lg transition-colors">
                  Agregar Juego
                </button>
              </form>
            </div>
                        ...
                        <tr className="border-t border-gray-600">
                          <td colSpan={5} className="px-6 py-4">
                            <div className="grid grid-cols-2 gap-4">
                              <div>
                                <label className="text-sm text-gray-400 mb-1 block">📖 Descripción</label>
                                <textarea value={editando.descripcion || ''} onChange={e => setEditando({...editando, descripcion: e.target.value})} placeholder="Descripción del juego..." rows={4} className="w-full bg-gray-700 text-white px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 resize-none text-sm" />
                              </div>
                              <div>
                                <label className="text-sm text-gray-400 mb-1 block">💻 Requisitos del sistema</label>
                                <textarea value={editando.requisitos || ''} onChange={e => setEditando({...editando, requisitos: e.target.value})} placeholder="SO: Windows 10&#10;RAM: 8GB..." rows={4} className="w-full bg-gray-700 text-white px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 resize-none text-sm" />
                              </div>
                            </div>
                          </td>
                        </tr>
